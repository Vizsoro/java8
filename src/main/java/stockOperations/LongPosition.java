...
public class LongPosition {
	...
	public static LongTransaction optimalLongTransaction(int[] a) {
		...
		Deque<PriceAndDay> optimal = transactions.stream().sorted(
				(d1, d2) -> (d2.peekFirst().price - d2.peekLast().price) - (d1.peekFirst().price - d1.peekLast().price))
				.findFirst().get();

		return new LongTransaction(optimal.peekLast().day, optimal.peekLast().price, optimal.peekFirst().day,
